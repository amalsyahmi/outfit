...
export const ModalProvider = ({children}: {children: ReactNode}) => {
  ...
  const actions = React.useRef({
    ...
    removeContent(uniqueId: Content['uniqueId']) {
      setContentList(list => list.filter(item => item.uniqueId !== uniqueId));
    },
...
export function Modal({children, blockHardwareBackButton = true}: ModalProps) {
  const uniqueId = useRef(Math.random().toString()).current;
  const actions = React.useContext(ModalContext);

  if (!actions) {
    throw new Error('Trying to use Modal without ModalProvider');
  }
